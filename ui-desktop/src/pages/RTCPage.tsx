...
export function RTCPage() {
  ...
  useEffect(() => {
    ...
    const bootstrap = async () => {
      try {
        ...
        const parsed = parseEndpoint(client.endpoint);
        if (active && parsed) {
  ...
  useEffect(() => {
    client.setEndpoint(buildEndpoint(signalingHost, signalingPort, normalizedSignalingPath));
  }, [signalingHost, signalingPort, normalizedSignalingPath]);
    ...
    <div className="grid gap-4">
      ...
      <section className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <Link className="text-sm font-medium text-sky-700 transition hover:text-sky-800" to="/logs">
            查看日志
          </Link>
        </div>
      ...
      <section className="grid gap-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <Link className="text-sm font-medium text-sky-700 transition hover:text-sky-800" to="/members">
            查看成员
          </Link>
        </div>
      ...
      <section className="grid gap-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="flex flex-wrap justify-end gap-2">
          ...
          />
          <Button variant="primary" onClick={() => fileInputRef.current?.click()} disabled={!canUseRtc}>
            选择并发送文件
          </Button>
        </div>

...
const FILE_CONNECTION_RECOVERY_TIMEOUT_MS = 20_000;

type SignalingClientContextValue = {
  endpoint: string;
  ...
  clearLogs: () => void;
  error: string;
  fileTransferRecoveryMessage: string;
  isRecoveringFileTransfer: boolean;
  connect: () => void;
  disconnect: () => void;
  connectRtcTo: (peerId: string) => void;
  closeRtc: () => void;
  sendRtcText: () => void;
  beginFileSelection: () => void;
...
export function SignalingClientProvider({ children }: { children: React.ReactNode }) {
  ...
  const assignedUserIdRef = useRef('');
  const onlineMembersRef = useRef<string[]>([]);
  const fileSelectionSnapshotRef = useRef<FileSelectionSnapshot | null>(null);
  const fileRecoveryAttemptRef = useRef(0);
  ...
  const [fileTransferRecoveryMessage, setFileTransferRecoveryMessage] = useState('');
  const [isRecoveringFileTransfer, setIsRecoveringFileTransfer] = useState(false);
  ...
  useEffect(() => {
    const onVisibilityChange = () => addLog('system', `页面可见性: ${document.visibilityState}`);
    const onPageHide = (event: PageTransitionEvent) => addLog('system', `页面隐藏: persisted=${event.persisted}`);
    const onPageShow = (event: PageTransitionEvent) => addLog('system', `页面显示: persisted=${event.persisted}`);
  ...
  const updateOnlineMembers = (payload: unknown) => {
    ...
    if (messageType === 'connectSignalingServerOk') {
      ...
      const nextMembers = [...new Set(members)].filter((member) => member !== selfId).sort();
      assignedUserIdRef.current = selfId;
      onlineMembersRef.current = nextMembers;
    ...
    if (messageType === 'memberJoined') {
      ...
      setOnlineMembers((prev) => {
        const next = prev.includes(nextUserId) ? prev : [...prev, nextUserId].sort();
    ...
    if (messageType === 'memberLeft') {
      ...
      setOnlineMembers((prev) => {
        const next = prev.filter((member) => member !== nextUserId);
        onlineMembersRef.current = next;
        return next;
      });
      setMemberRtcPeers((prev) => {
  ...
  const createTransferService = (rtc: RtcTextService) => {
    ...
    transfer.onDidStartUpload((file) => {
      addLog('system', `接收端已确认，开始发送: ${file.name}`);
    });
  ...
  const connectToEndpoint = (requestedEndpoint?: string) => {
    const nextEndpoint = (requestedEndpoint ?? endpointRef.current).trim();
    if (!nextEndpoint) {
    ...
    socket.onclose = (event) => {
      ...
      setOnlineMembers([]);
      setMemberRtcPeers({});
      addLog('system', `WebSocket 已关闭: code=${event.code} reason=${event.reason || '-'} wasClean=${event.wasClean}`);
    };
  };

  const connect = () => connectToEndpoint();

  const disconnect = () => {
    fileRecoveryAttemptRef.current += 1;
    fileSelectionSnapshotRef.current = null;
    setIsRecoveringFileTransfer(false);
    setFileTransferRecoveryMessage('');
    reportRtcPresence(null);
    socketRef.current?.close();
    socketRef.current = null;
    destroyRtc();
  ...
  const beginFileSelection = () => {
    fileSelectionSnapshotRef.current = {
      endpoint: endpointRef.current.trim(),
      peerId: rtcPeerIdRef.current,
    };
    addLog('system', '正在打开系统文件选择器');
  };

  const cancelFileSelection = () => {
    fileSelectionSnapshotRef.current = null;
    addLog('system', '已取消选择文件');
  };
  ...
  const sendFiles = async (files: FileList | null) => {
    ...
    const snapshot = fileSelectionSnapshotRef.current ?? {
      endpoint: endpointRef.current.trim(),
      peerId: rtcPeerIdRef.current,
    };
    fileSelectionSnapshotRef.current = null;
    const recoveryAttempt = fileRecoveryAttemptRef.current + 1;
    fileRecoveryAttemptRef.current = recoveryAttempt;
    setError('');

    try {
      const connectionStillUsable =
        socketRef.current?.readyState === WebSocket.OPEN && rtcRef.current?.isConnected() === true && rtcPeerIdRef.current === snapshot.peerId;

      if (!connectionStillUsable) {
        if (!snapshot.endpoint || !snapshot.peerId) {
          throw new Error('文件选择前没有可恢复的 RTC 连接');
        }

        setIsRecoveringFileTransfer(true);
        setFileTransferRecoveryMessage('正在恢复连接，文件将在连接成功后发送');
        addLog('system', `文件选择期间连接已断开，正在恢复与 ${snapshot.peerId} 的连接`);

        if (socketRef.current?.readyState !== WebSocket.OPEN || !assignedUserIdRef.current) {
          connectToEndpoint(snapshot.endpoint);
          await waitForCondition(
            () => fileRecoveryAttemptRef.current === recoveryAttempt && socketRef.current?.readyState === WebSocket.OPEN && Boolean(assignedUserIdRef.current),
            FILE_CONNECTION_RECOVERY_TIMEOUT_MS,
            '恢复信令连接超时',
          );
        }

        if (!onlineMembersRef.current.includes(snapshot.peerId)) {
          throw new Error(`对端 ${snapshot.peerId} 已离线，无法发送所选文件`);
        }

        const rtc = rtcRef.current;
        if (!rtc) {
          throw new Error('恢复后 RTC 服务未初始化');
        }

        if (!rtc.isConnected() || rtcPeerIdRef.current !== snapshot.peerId) {
          rtc.close();
          createTransferService(rtc);
          await rtc.connect(snapshot.peerId);
          await waitForCondition(
            () => fileRecoveryAttemptRef.current === recoveryAttempt && rtcRef.current === rtc && rtc.isConnected() && rtcPeerIdRef.current === snapshot.peerId,
            FILE_CONNECTION_RECOVERY_TIMEOUT_MS,
            '恢复 RTC 连接超时',
          );
        }
      ...
      if (fileRecoveryAttemptRef.current !== recoveryAttempt) {
        throw new Error('文件发送已取消');
      }
      const transfer = transferRef.current;
      if (!transfer || !rtcRef.current?.isConnected()) {
        throw new Error('文件传输服务未初始化或 RTC data channel 未连接');
      }
      await transfer.sendFiles(selectedFiles);
      addLog('system', `等待接收端确认: ${selectedFiles.length} 个文件`);
    } catch (err) {
      const message = String(err);
      setError(message);
      addLog('system', `文件发送失败: ${message}`);
    } finally {
      if (fileRecoveryAttemptRef.current === recoveryAttempt) {
        setIsRecoveringFileTransfer(false);
        setFileTransferRecoveryMessage('');
  ...
  const value: SignalingClientContextValue = {
    endpoint,
    setEndpoint,
    state,
    assignedUserId,
    ...
    clearLogs: () => setLogs([]),
    error,
    fileTransferRecoveryMessage,
    isRecoveringFileTransfer,
    connect,
    disconnect,
    connectRtcTo,
    closeRtc,
...
function waitForCondition(predicate: () => boolean, timeoutMs: number, timeoutMessage: string) {

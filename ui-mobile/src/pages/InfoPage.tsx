  ...
  return (
    <div className="grid gap-4">
      <header className="flex flex-wrap items-center justify-between gap-2">
        ...
        <Link className="inline-flex min-h-12 items-center text-sm font-medium text-sky-700 transition active:text-sky-800" to="/settings">
          返回设置
        </Link>
      </header>

      <section className="grid gap-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div className="-mx-1 flex gap-2 overflow-x-auto border-b border-slate-200 px-1 pb-3">
          {tabs.map((tab) => (
            <button
              key={tab.key}
              type="button"
              className={`min-h-12 shrink-0 rounded-xl px-3 py-2 text-sm font-medium transition ${
...
function InfoTable({ rows }: { rows: InfoRow[] }) {
  return (
    <div className="grid gap-3">
      {rows.map((row) => (
        <article className="grid gap-2 rounded-xl bg-slate-50 px-3 py-3 text-sm" key={row.key}>
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h3 className="font-medium text-slate-800">{row.zhName}</h3>
              <p className="break-all font-mono text-xs text-slate-500">{row.enName}</p>
            </div>
            <span
              className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${row.supported ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}
            >
              {row.supported ? '支持' : '不支持'}
            </span>
          </div>
          <p className="break-all border-t border-slate-200 pt-2 font-mono text-xs leading-5 text-slate-700">{row.value}</p>
        </article>
      ))}
    </div>
  );
